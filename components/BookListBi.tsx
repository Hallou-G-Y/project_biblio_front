...
export function BookList({ personalLibraryOnly = false }: BookListProps) {
  const { books, userLibrary, loading, error, addToPersonalLibrary, removeFromPersonalLibrary, updateBookStatus } = useLibrary();
  const { user } = useAuth();
  const [selectedBook, setSelectedBook] = useState<string | null>(null);
  const [showPersonalLibrary, setShowPersonalLibrary] = useState(personalLibraryOnly);

  const displayedBooks = showPersonalLibrary
    ? books.filter(book => userLibrary.some(lib => lib.bookId === book.googleBookId))
    : books;
  ...
  const getBookStatus = (bookId: string) => {
    ...
    return libraryBook?.status || 'to-read';
  };
    ...
    <>
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 3 }}>
        <Button
          ...
          onClick={() => setShowPersonalLibrary(!showPersonalLibrary)}
        >
              ...
              <Grid item xs={12} sm={6} md={4} key={book.googleBookId}>
                <Card>
                  <CardMedia
                    component="img"
                    height="300"
                    image={book.imageLinks?.thumbnail || '/placeholder-book.jpg'}
                    alt={book.title}
                    ...
                    {inLibrary && (
                      <FormControl fullWidth sx={{ mt: 2 }}>
                        ...
                        <Select
                          value={status}
                          label="Status"
                          onChange={(e) => updateBookStatus(book.googleBookId, e.target.value)}
                          size="small"
                        >
                  ...
                  <CardActions>
                    {showPersonalLibrary ? (
                      <Button
                        size="small"
                        color="error"
                        onClick={() => removeFromPersonalLibrary(book.googleBookId)}
                      >
                        Remove
                      </Button>
                    ) : (
